...
declare v_window_seconds integer; v_actor_limit integer; v_ip_limit integer; v_global_limit integer; v_window_started_at timestamptz; v_actor_hash text; v_ip_hash text; v_global_hash text; v_actor_allowed boolean; v_ip_allowed boolean; v_global_allowed boolean; v_retry_after_seconds integer; v_rule_found boolean:=false; v_affected integer;
begin
 for v_window_seconds,v_actor_limit,v_ip_limit,v_global_limit in select rr.window_seconds,rr.actor_limit,rr.ip_limit,rr.global_limit from private.feedback_rate_limit_rules as rr where rr.action=p_action loop v_rule_found:=true; exit; end loop; if not v_rule_found then raise exception using errcode='22023',message='invalid_rate_limit_action'; end if;
