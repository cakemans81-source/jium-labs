...
declare r private.feedback_rate_limit_rules%rowtype; w timestamptz; a text; i text; g text; aa boolean; ia boolean; ga boolean; retry integer;
begin
 select * into r from private.feedback_rate_limit_rules where action=p_action; if not found then raise exception using errcode='22023',message='invalid_rate_limit_action'; end if;
 w:=pg_catalog.to_timestamp(pg_catalog.floor(pg_catalog.extract(epoch from pg_catalog.clock_timestamp())/r.window_seconds)*r.window_seconds); retry:=pg_catalog.greatest(1,r.window_seconds-pg_catalog.floor(pg_catalog.extract(epoch from pg_catalog.clock_timestamp()-w))::integer);
 a:=private.feedback_hmac('actor:'||p_actor); i:=private.feedback_hmac('ip:'||p_ip); g:=private.feedback_hmac('global:'||p_action);
 with x as (insert into private.feedback_rate_limit_windows as q values('actor',a,p_action,w,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<r.actor_limit returning 1) select exists(select 1 from x) into aa;
 with x as (insert into private.feedback_rate_limit_windows as q values('ip',i,p_action,w,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<r.ip_limit returning 1) select exists(select 1 from x) into ia;
 with x as (insert into private.feedback_rate_limit_windows as q values('global',g,p_action,w,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<r.global_limit returning 1) select exists(select 1 from x) into ga;
 return query select aa and ia and ga,case when aa and ia and ga then 0 else retry end;
end; $$;
...
revoke all on function private.feedback_hmac(text,text),private.feedback_actor_hashes(text),private.feedback_rate_limit_consume(text,text,text),private.feedback_valid_actor_ip(text,text),private.feedback_canonical_ip(text),public.sync_feedback_comment_count(),public.feedback_create(text,text,text,text,text,text,text),public.feedback_comment_create(text,text,text,text,text),public.feedback_vote_set(text,boolean,text,text),public.feedback_vote_state(text,text,text) from public,anon,authenticated;
