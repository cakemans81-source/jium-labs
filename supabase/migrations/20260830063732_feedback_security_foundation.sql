...
declare v_window_seconds integer; v_actor_limit integer; v_ip_limit integer; v_global_limit integer; v_window_started_at timestamptz; v_actor_hash text; v_ip_hash text; v_global_hash text; v_actor_allowed boolean; v_ip_allowed boolean; v_global_allowed boolean; v_retry_after_seconds integer;
begin
 select rr.window_seconds,rr.actor_limit,rr.ip_limit,rr.global_limit into v_window_seconds,v_actor_limit,v_ip_limit,v_global_limit from private.feedback_rate_limit_rules as rr where rr.action=p_action; if not found then raise exception using errcode='22023',message='invalid_rate_limit_action'; end if;
 ...
 with x as (insert into private.feedback_rate_limit_windows as q values('actor',v_actor_hash,p_action,v_window_started_at,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<v_actor_limit returning 1) select exists(select 1 from x) into v_actor_allowed;
 with x as (insert into private.feedback_rate_limit_windows as q values('ip',v_ip_hash,p_action,v_window_started_at,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<v_ip_limit returning 1) select exists(select 1 from x) into v_ip_allowed;
 with x as (insert into private.feedback_rate_limit_windows as q values('global',v_global_hash,p_action,v_window_started_at,1) on conflict(scope,scope_hash,action,window_started_at) do update set request_count=q.request_count+1 where q.request_count<v_global_limit returning 1) select exists(select 1 from x) into v_global_allowed;
 return query select v_actor_allowed and v_ip_allowed and v_global_allowed,case when v_actor_allowed and v_ip_allowed and v_global_allowed then 0 else v_retry_after_seconds end;
end; $$;
